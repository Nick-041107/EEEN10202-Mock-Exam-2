...
start: ; ??????part
  ...
  movlw 0x0F
  movwf ADCON1, a ; ???????
  bsf TRISB, 0, a ; ??((PORTB) and 0FFh), 0, a?[???((PORTB) and 0FFh), 0, a?]
  ...
  movlw 11101111B ; ??((PORTA) and 0FFh), 4, a?[Transistor Q3)
  movwf TRISA, a


main: ; ????part
  clrf LATF, a ; !!! ??LED ????????0
  clrf LATA, a ; !!! ??((PORTA) and 0FFh), 4, a???
  ...
  movwf LATF, a ; ?????PORT F?
  bsf LATA, 4, a ; ??((PORTA) and 0FFh), 4, a
  clrf LATA, a ; ??((PORTA) and 0FFh), 4, a

  ;7-sgement ?((PORTB) and 0FFh), 0, a Part
...
pb2_pressed:
  bcf LATH, 1, a ; !!! ((PORTH) and 0FFh), 0, a?1 ?0???
